...
def load_excel_courier_base_rates(session_id: str, parameter_revision: int = 0) -> pd.DataFrame:
    ...
    if not stop_count_bonus.empty:
        ...
        result["Cím bónusz (Kifli)"] = (
            ...
            .fillna(result["_courier_lookup"].map(by_name))
            .fillna(0.0)
        )
...
def load_session_payable_totals(session_id: str | None) -> dict[str, float]:
    ...
    for item in rows:
        amount = (
            parse_huf_value(item.get("payable_huf"))
            or parse_huf_value(item.get("payable_total_huf"))
            or parse_huf_value(item.get("calculated_total"))
        )
        courier_key = _courier_id_key(item.get("courier_id"))
        courier_name_key = _courier_match_key(item.get("driver_name"))
        if courier_key:
            totals[courier_key] = amount
            totals[f"id:{courier_key}"] = amount
        if courier_name_key:
            totals[f"name:{courier_name_key}"] = amount
    return totals
...
def lookup_session_payable_total(totals: dict[str, float], courier_id: object, courier_name: object = "") -> float:
    ...
    if courier_key:
        if f"id:{courier_key}" in totals:
            return totals[f"id:{courier_key}"]
        if courier_key in totals:
            return totals[courier_key]
    courier_name_key = _courier_match_key(courier_name)
    if courier_name_key:
        return totals.get(f"name:{courier_name_key}", 0.0)
    return 0.0
...
def loyalty_advance_booking_min_from_rule(rule: pd.Series) -> int:
    """The old DB column name is kept for compatibility; it means advance-booked shifts."""
    for column in (
...
def courier_detail_menu_for_status(status: object) -> str:
    normalized = str(status or "").strip().casefold()
    if not normalized:
        return "Pénzügy"
    if "fizetés előleg" in normalized:
        return "Fizetés előleg"
    if normalized in {"kifizetésre vár", "kifizetve"}:
        return "Kifizetés"
    if "bejelentés" in normalized or "reklam" in normalized:
        return "Reklamációk"
    if (
        "elszámolás" in normalized
        or "tig" in normalized
        or "számla" in normalized
    ):
        return "Dokumentumok"
    return "Pénzügy"
...
def render_courier_detail_page() -> None:
    ...
    if selected_menu == "Reklamációk":
        ...
        with complaint_editor:
            st.markdown("##### Reklamáció kezelése")
            if complaints.empty:
                ...
                selected_message = str(selected_complaint.get("message") or "").strip()
                selected_status_label = complaint_status_labels.get(
                    str(selected_complaint.get("status") or "").strip(),
                ...
                if st.button("Lezárom", type="primary", use_container_width=True, key=f"ui_complaint_close_{courier_id}_{selected_complaint_id}"):
                    try:
                        ...
                        st.success("Reklamáció lezárva. A válasz mentve lett.")
                        st.rerun()
                    except Exception as exc:
