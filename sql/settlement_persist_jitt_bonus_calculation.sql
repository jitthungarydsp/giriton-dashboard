...
-- Persist every route-level settlement component on the existing JITT rows.
-- The UI must only display these values; it must not independently calculate
-- Delay/Compliance bonuses from the raw Excel JSON.
...
grant select on settlement.vw_parameterized_courier_settlement_summary to service_role;

-- Populate the new persisted values for already imported sessions as well.
