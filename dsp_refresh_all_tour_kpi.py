...
9. public.dsp_tour_compliance
10. public.dsp_driver_tour_kpi_monthly
...
CREATE_TABLES_SQL = dedent(
    ...
    create table if not exists public.dsp_tour_compliance (
        work_date date not null,
        work_month date not null,
        ...
        planned_route_minutes integer,
        real_route_minutes integer,
        route_duration_diff_minutes integer,

        no_show boolean not null default false,
        late_departure boolean not null default false,
        late_departure_minutes integer,
        tour_status text,
    ...
    create table if not exists public.dsp_driver_tour_kpi_monthly (
        work_month date not null,
        courier_id integer not null,
        courier_name text,
        warehouse_name text,

        accepted_tours integer not null default 0,
        completed_tours integer not null default 0,
        no_show_tours integer not null default 0,
        no_show_percent numeric(10,2),

        started_tours integer not null default 0,
        late_start_tours integer not null default 0,
        late_start_percent numeric(10,2),

        tour_compliance_percent numeric(10,2),
        tour_compliance_level text,

        total_orders integer not null default 0,
        late_orders integer not null default 0,
        order_delay_percent numeric(10,2),
        delay_level text,

        average_planned_route_minutes numeric(10,2),
        average_real_route_minutes numeric(10,2),

        created_at timestamptz default now(),
        updated_at timestamptz default now(),

        primary key (work_month, courier_id)
    );
...
UPSERT_TOUR_COMPLIANCE_SQL = dedent(
    """
    insert into public.dsp_tour_compliance (
        work_date,
        work_month,
        courier_id,
        courier_name,
    ...
    select
        s.work_date,
        date_trunc('month', s.work_date)::date as work_month,
        s.courier_id,
        s.courier_name,
        s.warehouse_name,
        ...
        r.real_route_minutes,
        case
            when r.planned_route_minutes is null
              or r.real_route_minutes is null
                then null
            else r.real_route_minutes - r.planned_route_minutes
        end as route_duration_diff_minutes,

        (
            r.route_id is null
            or r.real_departure is null
        ) as no_show,

        (
            r.route_id is not null
            and r.real_departure is not null
            and r.planned_departure is not null
            and r.real_departure > r.planned_departure
        ) as late_departure,

        case
            when r.real_departure is null
              or r.planned_departure is null
                then null
            else round(
                extract(epoch from (r.real_departure - r.planned_departure)) / 60
            )::integer
        end as late_departure_minutes,

        case
            when r.route_id is null then 'Elfogadott, de nincs túra'
            when r.real_departure is null then 'Nem teljesített túra'
            when r.planned_departure is null then 'Nincs tervezett indulás'
            when r.real_departure > r.planned_departure then 'Későn megkezdett túra'
            else 'Időben megkezdett túra'
        end as tour_status,
    ...
    left join lateral (
        select route_row.*
        from public.dsp_attendance_routes route_row
        where route_row.work_date = s.work_date
          and route_row.courier_id = s.courier_id
          and (
              (
                  route_row.courier_registered_at is not null
                  and route_row.courier_registered_at
                      between s.shift_start - interval '90 minutes'
                          and s.shift_end + interval '90 minutes'
              )
              or (
                  route_row.planned_departure is not null
                  and route_row.planned_departure
                      between s.shift_start - interval '90 minutes'
                          and s.shift_end + interval '90 minutes'
              )
          )
        order by
            case
                when route_row.courier_registered_at is not null
                  and s.available_for_shift_since is not null
                    then abs(
                        extract(
                            epoch from (
                                route_row.courier_registered_at
                                - s.available_for_shift_since
                            )
                        )
                    )
                when route_row.planned_departure is not null
                  and s.shift_start is not null
                    then abs(
                        extract(
                            epoch from (
                                route_row.planned_departure
                                - s.shift_start
                            )
                        )
                    )
    ...
    do update set
        ...
        planned_route_minutes = excluded.planned_route_minutes,
        real_route_minutes = excluded.real_route_minutes,
        route_duration_diff_minutes = excluded.route_duration_diff_minutes,

        no_show = excluded.no_show,
        late_departure = excluded.late_departure,
        late_departure_minutes = excluded.late_departure_minutes,
...
UPSERT_DRIVER_TOUR_KPI_MONTHLY_SQL = dedent(
    """
    insert into public.dsp_driver_tour_kpi_monthly (
        work_month,
        courier_id,
        courier_name,
        warehouse_name,
        ...
        tour_compliance_percent,
        tour_compliance_level,
        ...
        average_planned_route_minutes,
        average_real_route_minutes,
    ...
    with summary as (
        select
            ...
            count(*) as accepted_tours,

            count(*) filter (
                where no_show = false
                  and route_id is not null
                  and real_departure is not null
            ) as completed_tours,
            ...
            count(*) filter (
                where late_departure = true
            ) as late_start_tours,

            coalesce(sum(orders), 0)::integer as total_orders,
            coalesce(sum(keso_rendelesek), 0)::integer as late_orders,

            round(avg(planned_route_minutes), 2)
                as average_planned_route_minutes,

            round(avg(real_route_minutes), 2)
                as average_real_route_minutes

        from public.dsp_tour_compliance
        group by work_month, courier_id
    ),
    calculated as (
        select
            *,
            ...
            round(
                no_show_tours::numeric
                / nullif(accepted_tours, 0)
                * 100,
                2
            ) as no_show_percent,

            round(
                late_start_tours::numeric
                / nullif(started_tours, 0)
                * 100,
                2
            ) as late_start_percent,
    ...
    scored as (
        select
            *,
            ...
            round(
                0.7 * coalesce(no_show_percent, 0)
                + 0.3 * coalesce(late_start_percent, 0),
                2
            ) as tour_compliance_percent
    ...
    select
        ...
        case
            when tour_compliance_percent <= 2.00 then 'Szint 1'
            when tour_compliance_percent <= 4.00 then 'Szint 2'
            when tour_compliance_percent <= 10.00 then 'Szint 3'
            else 'Nincs túramegfelelési díj'
        end as tour_compliance_level,
        ...
        case
            when order_delay_percent <= 1.50 then 'Szint 1'
            when order_delay_percent <= 3.00 then 'Szint 2'
            when order_delay_percent <= 5.00 then 'Szint 3'
            else 'Nincs késedelmi díj'
        end as delay_level,
    ...
    on conflict (work_month, courier_id)
    do update set
        courier_name = excluded.courier_name,
        warehouse_name = excluded.warehouse_name,

        accepted_tours = excluded.accepted_tours,
        completed_tours = excluded.completed_tours,
        no_show_tours = excluded.no_show_tours,
        no_show_percent = excluded.no_show_percent,

        started_tours = excluded.started_tours,
        late_start_tours = excluded.late_start_tours,
        late_start_percent = excluded.late_start_percent,

        tour_compliance_percent = excluded.tour_compliance_percent,
        tour_compliance_level = excluded.tour_compliance_level,

        total_orders = excluded.total_orders,
        late_orders = excluded.late_orders,
        order_delay_percent = excluded.order_delay_percent,
        delay_level = excluded.delay_level,

        average_planned_route_minutes = excluded.average_planned_route_minutes,
        average_real_route_minutes = excluded.average_real_route_minutes,
...
CHECK_SQL = dedent(
    ...
    union all
    select 'dsp_tour_compliance', count(*)
    from public.dsp_tour_compliance

    union all
    select 'dsp_driver_tour_kpi_monthly', count(*)
    from public.dsp_driver_tour_kpi_monthly
...
PIPELINE = [
    ...
    ("Túramegfelelési ténytábla", UPSERT_TOUR_COMPLIANCE_SQL),
    ("Futár havi késedelmi és túramegfelelési KPI", UPSERT_DRIVER_TOUR_KPI_MONTHLY_SQL),
]
